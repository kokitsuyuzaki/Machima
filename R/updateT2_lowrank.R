...
.updateT2_lowrank <- function(W_RNA, X_Epi, H_Sym, U, V, Beta, L1_T, L2_T, root,
    W_hic=NULL, h_hic=numeric(0), U_coup=NULL){
    if(is.matrix(X_Epi)){
        uv <- .updateT2_lowrank_Matrix(W_RNA, X_Epi, H_Sym,
            U, V, Beta, L1_T, L2_T, root,
            W_hic_k=W_hic, h_hic=h_hic, U_k=U_coup)
    }else{
        uv <- .updateT2_lowrank_List(W_RNA, X_Epi, H_Sym,
            U, V, Beta, L1_T, L2_T, root,
            W_hic=W_hic, h_hic=h_hic, U_coup=U_coup)
    }
    uv
}
...
.updateT2_lowrank_Matrix <- function(W_RNA, X_Epi, H_Sym,
    ...
    numer_V <- W_E %*% H_Sym %*% GtSbXU                 # n x r
    Sb_U <- S_hat^(Beta - 1) %*% U                      # l x r
    GtSbU <- t(G) %*% Sb_U                              # J x r
    denom_V <- W_E %*% H_Sym %*% GtSbU + L1_T + L2_T * V  # n x r
...
.updateT2_lowrank_List <- function(W_RNA, X_Epi, H_Sym,
    ...
    K <- length(X_Epi)
    for(k in seq_len(K)){
        W_E <- if(!is.null(U_coup)) W_RNA[[k]] + U_coup[[k]] else W_RNA[[k]]
        TT_k <- U[[k]] %*% t(V[[k]])
        G <- TT_k %*% W_E
        S_hat <- G %*% H_Sym %*% t(G)
        ...
        GtSbXU <- t(G) %*% SbX_U
        numer_V <- W_E %*% H_Sym %*% GtSbXU
        Sb_U <- S_hat^(Beta - 1) %*% U[[k]]
        GtSbU <- t(G) %*% Sb_U
        denom_V <- W_E %*% H_Sym %*% GtSbU + L1_T + L2_T * V[[k]]
